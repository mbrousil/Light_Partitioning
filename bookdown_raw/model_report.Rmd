...
params:
  ...
  simultaneous_data_no_sdd: NA
  simultaneous_data_strict: NA
  simultaneous_data_strict_no_sdd: NA
  simultaneous_data_middle: NA
  simul_vis_gg_mr: NA
  simul_vis_gg: NA
...
# Our attempt at recreating MR's data - no secchi
simultaneous_data_no_sdd <- params$simultaneous_data_no_sdd
# A stricter version
simultaneous_data_strict <- params$simultaneous_data_strict
# Strict version without secchi
simultaneous_data_strict_no_sdd <- params$simultaneous_data_strict_no_sdd
# Our attempt at recreating MR's data - middle 96% only
simultaneous_data_middle <- params$simultaneous_data_middle
```
...
**Our dataset, no secchi**:

```{r echo=FALSE, message = F}
simultaneous_data_no_sdd %>%
  pivot_longer(names_to = "measure", values_to = "value", c(chla, doc, tss)) %>%
  ggplot() +
  geom_histogram(aes(value), color = "black", fill = "white") +
  facet_wrap(vars(measure), scales = "free") +
  ggtitle("Current dataset, no secchi") +
  theme_bw()
```

**Our dataset, no 0.001 or 0.999**:
...
simultaneous_data_middle %>%
  ...
  geom_histogram(aes(value), color = "black", fill = "white") +
  facet_wrap(vars(measure), scales = "free") +
  ggtitle("Current dataset, no 0.001 or 0.999") +
  theme_bw()
```
...
simultaneous_data_strict_no_sdd %>%
  ...
  geom_histogram(aes(value), color = "black", fill = "white") +
  facet_wrap(vars(measure), scales = "free") +
  ggtitle("Strict version of current dataset, no secchi") +
  theme_bw()
```
...
bind_rows(
  ...
  simultaneous_data_no_sdd %>%
    pivot_longer(names_to = "parameter", values_to = "value", c(chla, doc, tss)) %>%
    group_by(parameter) %>%
    summarize(dataset = "Current dataset, no secchi",
              mean_val = mean(value, na.rm = TRUE),
              sd_val = sd(value, na.rm = TRUE)),
  ...
  simultaneous_data_middle %>%
    pivot_longer(names_to = "parameter", values_to = "value", c(chla, doc, secchi, tss)) %>%
    group_by(parameter) %>%
    summarize(dataset = "Current dataset, no 0.001 or 0.999",
              mean_val = mean(value, na.rm = TRUE),
              sd_val = sd(value, na.rm = TRUE)),
  ...
  simultaneous_data_strict_no_sdd %>%
    pivot_longer(names_to = "parameter", values_to = "value", c(chla, doc, tss)) %>%
    group_by(parameter) %>%
    summarize(dataset = "Strict dataset, no secchi",
              mean_val = mean(value, na.rm = TRUE),
              sd_val = sd(value, na.rm = TRUE))
) %>%
...
We'll run the linear model with all versions of the dataset. The code below is adapted from MR's scripts.
```{r}
range_val <- 234
...
model_metrics %>%
  left_join(x = .,
            y = tibble(
              dataset = c("simul", "simultaneous_data", "simultaneous_data_no_sdd",
                          "simultaneous_data_middle", "simultaneous_data_strict",
                          "simultaneous_data_strict_no_sdd"),
              rows = c(nrow(simul), nrow(simultaneous_data), nrow(simultaneous_data_no_sdd),
                       nrow(simultaneous_data_middle), nrow(simultaneous_data_strict),
                       nrow(simultaneous_data_strict_no_sdd))),
            by = c("dataset")) %>%
  kable() %>%
  kable_paper()
```
